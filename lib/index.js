...
var defaultOptions = {
	...
	mongodb: 'mongodb://localhost/scoped_registry',
...
exports.register = function (plugin, pluginOptions, next) {
	Hoek.assert(pluginOptions.secret, 'secret for token encryption required');
		...
		plugin.route({
			...
			config: {
				...
				handler: function (request, reply) {
					...
					if (filename.indexOf(packageName) !== 0) {
						return reply(Hapi.error.badRequest('Package and filename doesn\'t match'));
					} else if (!internals.canInstall(packageName, request.auth.credentials)) {
						return reply(Hapi.error.forbidden('Not allowed to access this package'));
					} else {
						registry.fetchArchive(filename, function (error, stream) {
							if (error) {
